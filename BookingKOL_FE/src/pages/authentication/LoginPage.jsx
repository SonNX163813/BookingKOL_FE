...
export default function LoginPage() {
  ...
  // ⚠️ Fallback an toàn phòng khi thiếu Provider (không bắt buộc nếu App đã bọc AuthProvider)
  const auth = useAuth?.() || {};
  ...
  // ✅ Nếu đã đăng nhập (đã có token) thì tự đẩy về trang trước hoặc "/"
  useEffect(() => {
    if (token) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // server của bạn trả { data: { accessToken, id, roles, ... } }
      const token =
        data?.accessToken ||
      ...
      // Lưu vào context + remember để sync local/session storage
      dispatch({
        type: "LOGIN_SUCCESS",
