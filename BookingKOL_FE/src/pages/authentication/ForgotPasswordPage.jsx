...
export default function ForgotPasswordPage() {
  ...
  const handleRequest = async (e) => {
    ...
    try {
      ...
      // ====== CHỈ THÀNH CÔNG KHI CÓ "TÍN HIỆU OK" ======
      // Các tín hiệu OK có thể là:
      // - res.ok && (data.success === true || data.status === 200 || data.code === 0)
      // - hoặc message chứa từ khóa "Đã gửi OTP"/"Gửi OTP thành công"
      ...
      // ====== Các tín hiệu LỖI phổ biến từ BE ======
      ...
      if (!looksSuccess || looksEmailNotFound) {
        // Ở đây coi như THẤT BẠI → không chuyển bước
        setRequestErr(
          looksEmailNotFound
      ...
      // Thành công → chuyển sang bước VERIFY
      setStep("VERIFY");
      setOkMsg(
        msgStr ||
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      try {
        ...
        data = null; // BE có thể trả rỗng / không phải JSON
      }
