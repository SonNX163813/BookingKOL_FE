...
import { adminGetBookingRequestsByUser } from "../../../../services/admin/AdminBookingAPI";
import { useAdminGetViewProfileUser } from "../../../../hook/admin/management-user/useAdminGetViewProfileUser";

const { RangePicker } = DatePicker;

/* ===== dayjs Việt hoá trong file ===== */
...
export default function AdminUserBookingById() {
  ...
  // Lấy info user để hiển thị tên
  const { isLoadingGetViewProfileUser, ResponseGetViewProfileUser } =
    useAdminGetViewProfileUser(userId);

  const userName =
    pickValue(ResponseGetViewProfileUser, [
      "fullName",
      "displayName",
      "email",
    ]) || `#${userId}`;

  // Gọi API booking theo userId
  const { data, isPending, isFetching, refetch } = useQuery({
    ...
    queryFn: () =>
      adminGetBookingRequestsByUser(userId, {
        page,
        size,
        ...filters,
      }),
  ...
  const handleBack = () => {
    ...
    else navigate("/admin/management-customer");
  };
            ...
            <div>
              <h1 className="text-[18px] font-bold">
                ...
                <span className="uppercase">
                  {isLoadingGetViewProfileUser ? "..." : userName}
                </span>
              </h1>
              <p className="text-[12px] text-gray-500">User ID: {userId}</p>
            </div>
          </div>
        </div>
      </div>
